...
class ValidationEngine:
    ...
    def _validate_sheet(self, df, sheet_name, accepted_responses, row_offset=1):
        ...
        for col, spec in accepted_responses.items():
            ...
            if col_type == "categorical" or col_type == "multiCategorical": # Added or multiCategorical to test. If fails, can create its own if clause
                ...
                min_wage = spec.get("min_wage")
                max_wage = spec.get("max_wage")
                validator = cls(required = spec.get("required", False),
                    row_numbers = df[f"row_number_{sheet_name}"],
                    min_wage = min_wage,
                    max_wage = max_wage
                    )
            ...
            normalized_cols[col] = s_fmt # serves as default for non-multiCategorical cols and serves to preserve original col for multiCategorical col

            # for multiCategorical columns, generate indicators and add to normalized_cols (and later normalized_df) - doing this here because it relies on the accepted responses and the normalized values, which are both generated by the column validator
            if col_type == "multiCategorical":
                ...
                # sanity check for indicator generation - if this fails, likely a problem in the accepted responses or the indicator function
                if num_indicators != num_accepted_values:
                    warnings.warn(
                        f"Number of indicator columns ({num_indicators}) does not match number of accepted values ({num_accepted_values}) for column '{col}' in sheet '{sheet_name}'."
                    )
    ...
    def validate_workbook(self, file, workbook_type, workbook_format, dfs_by_sheet, passed_identity_sheet, keycreators = None):
        ...
        if self.logging:
            ## create a unique id connected to the information passed in the run table 
            self.run_id =  self.db_logger.start_run(workbook_type, self.org, self.quarter, triggered_by="rdaly", run_description = self.log_description)
        ...
        if self.logging:
            ...
            for _, row in self.single_sheet.iterrows():
                ...
                participant_id = self.db_logger.get_or_create_participant( # todo: possibly add logic here for matching
                    person_id=pid,

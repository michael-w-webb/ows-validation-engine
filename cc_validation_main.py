...
for org, data_types in cc_file_directory.items():
    ...
    if target_period not in periods: # Daly: uncommented this in order to obtain data for previous quarters.
        continue
    period = target_period

    # period = periods[-1]  # pick latest period for processing. Daly: commented this out in order to obtain data for previous quarters.
...
output_file = rf"{DATA_DIR}\Career_ConneCT\.Programmatic_Data\Cleaned Programmatic Data\cc_validation_results_all_orgs_{target_period}.xlsx"
